main.path="/media/monika/683375f9-7173-4a68-ba60-f33b0c47a470/Analysis/Analysis_of_scRNAseq_using_R"
setwd(main.path)
...
write.table(all.counts, file ="data/all.counts.txt", quote = FALSE, sep = "\t")

colnames(endo.data$metadata)[2] <- "group"
colnames(endo.data$metadata)[3] <- "mRNA"
colnames(endo.data$metadata)[8] <- "cellId"
write.table(endo.data$metadata, file ="data/metadata.txt", quote = FALSE, sep = "\t", row.names = FALSE)
